...
void totalTagihan(int index, int total) {
    if (index == daftarPengunjung.size()) {
        cout << "Total Tagihan untuk Semua Pengunjung: Rp." << total << endl;
        return;
    }
        int harga_per_jam;
    if (daftarPengunjung[index].jenis_ps == "PS3") {
        harga_per_jam = 6000;
    } else if (daftarPengunjung[index].jenis_ps == "PS4") {
        harga_per_jam = 8000;
    } else if (daftarPengunjung[index].jenis_ps == "PS5") {
        harga_per_jam = 12000;
    } else {
        cout << "Jenis PS tidak valid untuk pengunjung: " << daftarPengunjung[index].nama << endl;
        totalTagihan(index + 1, total); // Rekursi terjadi di sini
        return;
    }

    int tagihan = harga_per_jam * daftarPengunjung[index].lama_peminjaman;
    total += tagihan;
    totalTagihan(index + 1, total); // Rekursi terjadi di sini
}
...
int main() {
    ...
    while (true) {
        ...
        switch (pilihanUtama) {
            ...
            case 3:
                totalTagihan(0, 0); // memanggil fungsi rekursif untuk menampilkan total tagihan
